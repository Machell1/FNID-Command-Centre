...
from .. import db
from .types import GEOGRAPHY, JSONB, UUID
...
class DCRREntry(db.Model):
    ...
    entry_color = Column(
        String(10),
        CheckConstraint("entry_color IN ('BLUE', 'BLACK', 'RED')"),
        default='BLUE',
    )
...
class StationRegister(db.Model):
    ...
    register_type = Column(
        String(10),
        CheckConstraint("register_type IN ('MAJOR', 'MINOR')"),
        nullable=False,
    )
...
class CaseFileMovement(db.Model):
    ...
    movement_status = Column(
        String(15),
        CheckConstraint("movement_status IN ('ISSUED', 'OVERDUE', 'RETURNED', 'LOST', 'DAMAGED')"),
        default='ISSUED',
    )
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
...
class Investigation(db.Model):
    ...
    def __init__(self, **kwargs):
        ...
        if self.io_assigned_at is None:
            self.io_assigned_at = datetime.utcnow()
        if self.preliminary_vetting_due is None:
            self.preliminary_vetting_due = self.io_assigned_at + timedelta(hours=72)
        if self.next_review_due is None:
            self.next_review_due = self.io_assigned_at + timedelta(days=7)
...
class AuditLog(db.Model):
    ...
    officer_reg_number = Column(String(10), ForeignKey('personnel.reg_number'))
    officer_ip = Column(String(45), nullable=False)
    officer_device_fingerprint = Column(String(64))
    ...
    action_at = Column(DateTime(timezone=True), nullable=False, default=datetime.utcnow)
